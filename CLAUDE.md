# Loan Marketplace — Style Guide & Build Brief

2026-09-22 · @Jeremy Tan

## Part 1 — Visual system

## Design direction

Copy Mercury's restraint and rhythm, not its darkness. [Mercury](https://mercury.com) runs a dark-first marketing site with a proprietary typeface, one accent colour, cinematic photography and very few UI elements per screen. The restraint is the transferable part; the dark palette is a positioning choice that suits a US startup-banking brand and works against trust signals in Singapore consumer lending. Build light-first, with dark bands reserved for the hero and the closing CTA.

The patterns worth lifting, in the order they appear on Mercury's homepage:

1. **Full-bleed hero with a scroll-scrubbed video**, one display headline, one line of subhead, an inline email capture next to the primary CTA, and a quiet secondary link beside it.
...
3. **Eyebrow + H2 pattern** for the more functional sections: a small uppercase kicker, a large H2, and a text link to the deeper page on the same row.
4. **Feature grids of 2 or 4**, each cell = one product image, an H3 of 3–6 words, and one or two sentences. Never more.
5. **Testimonial blocks with full-bleed portrait photography** — a category tag, the quote in display type, then name and role. Real photography, art-directed, never stock headshots.
...
- **Motion is slow and linear.** Scroll-scrub on the hero, 400–600ms fade-and-rise reveals with `ease-out`, 12–16px of travel. No bounce, no spring, no parallax on text. Respect `prefers-reduced-motion`.
- **Abstract renders and real product UI only.** No photography, no mascot, no character art, no flat vector, no icon sets, no emoji. The image system is specified in full below.

### The image system

Here is every non-logo image on [Mercury's homepage](https://mercury.com), taken from their own alt text, so the coder and the image generator both work from the real inventory rather than an impression of it.

| Mercury's image | What it is | Our equivalent |
| --- | --- | --- |
| Multiple points converging into a single focal point | Abstract render, "simplicity and streamlined finances" | Many lender nodes converging into one application |
| Overlapping circles | Abstract render, "a unified view of incoming and outgoing funds" | Overlapping discs — every offer in one view |
| Three dots at the centre | Abstract render, "conversation, guidance, support" | Support and guidance |
| Three toggle switches | Abstract render, "controls for permissions, approvals, access" | Consent and data controls |
| A bar graph showing coverage vs traditional banks | Abstract render of a comparison | Your rate against the market average |
| A stylised graph with an upward trajectory | Abstract render, growth over time | Total cost over tenure |
| A credit card with points rippling above and below | Abstract render of a mechanism | Interest accruing over a repayment schedule |
| Close-up of the corner of a Mercury card | Stylised product macro | Not applicable — we have no physical product |
| The invoicing flow with dates and payment terms | Cropped fragment of the real product UI | Cropped fragment of the comparison screen |
| Rotating stack of virtual cards | Looping animation of the same render language | Offer cards fanning and settling into a sorted stack |
| Fingerprint | Abstract render, passkey authentication | Singpass and data security |
| Founder portraits beside testimonials | Real photography | **The one thing we drop** — see below |

So Mercury's system is three things, and only three: **abstract geometric renders**, **cropped fragments of the real product interface**, and **photography reserved exclusively for testimonials**.

**How the renders are made.** Each one carries a single idea, built from primitives — points, discs, bars, lines, toggles — rendered dimensionally with soft studio lighting and a shallow gradient, floating on a plain ground in the brand palette. They are not flat vector, not line icons, not glossy 3D clipart, and they contain no text, no arrows and no literal objects. Two or three tones from the palette each, at most. If someone can describe the image in one short sentence, it is right; if the sentence needs an "and", the image is doing too much.

**Cropped UI fragments are our strongest asset and we have more of them than Mercury does.** A comparison marketplace has a genuinely interesting interface — offer cards, a sort control, a rate column. Crop it hard, enlarge it past life size, sit it on a `--surface-1` or `--ink-900` ground with generous margin, and let it bleed off one edge. Use these wherever a section is about how the product works.

**On testimonials:** Mercury's only use of photography is founder portraits, and it is doing real work — it is what makes the quotes credible. Dropping photography means our testimonials need a different treatment: the quote set large in `display-md` on an `--ink-900` band, with the name, role and company beneath in `body-sm`. No portrait, no avatar, no initial-in-a-circle. Typography alone. This is harder to make convincing, so use two quotes at most and make them specific and numerate.
...
| Out | In |
| --- | --- |
| Mascot, character art, spot illustration, icon sets, photography | Abstract geometric renders and cropped fragments of the real interface |
...
| Imagery | Mascot illustrations | Art-directed photography of real Singaporeans |
...
Twelve sections in this order. Copy below is ready to use — treat it as final draft, not placeholder. Bracketed items are numbers to confirm before launch.
...
Full-bleed `--ink-900` band, 88vh desktop / auto mobile. Background: a slow looping render of many points converging into one, scroll-scrubbed the way Mercury scrubs its hero video — the motion advances with the scroll position rather than playing on a timer. Export as a short muted MP4 with a still first frame as the poster image. Text left-aligned, not centred.
...
- **Four cells**, each an image at 3:2, an H3, and one sentence:
...
- **Testimonials need consent on file.** Named quotes with photographs require written permission. Use initials and no photo otherwise.
...
Paste this document whole as the first message, then send this instruction:

> Build only the homepage. First set up the design tokens, the type scale and the base components exactly as specified — do not improvise sizes, colours or spacing values. Then build the thirteen homepage sections in order using the copy as written. Use placeholder images at the stated aspect ratios where real photography is not available yet. Do not build any other page; leave the nav and footer links pointing to the routes in the sitemap as dead links for now. When you are done, list every value you had to invent because it was not specified.
...
**Assets — use the Higgsfield MCP.** Generate the abstract renders through Higgsfield. No photography, no flat vector, no icons, no people. Everything else on the site is a cropped fragment of the real interface, which the coder renders rather than generates. What the homepage needs:

| Asset | Spec | Direction |
| --- | --- | --- |
| Hero loop | 16:9, 6–8s, muted MP4 + still poster | Many small points drifting inward and settling into one focal point. Dimensional, soft studio light, `--ink-900` ground, points in `--accent` and `--on-dark-900`. Slow. Scroll-scrubbed, not autoplaying. |
| Value grid renders (×4) | 3:2 desktop, 4:5 mobile | One idea each, built from primitives: (1) points converging into one — one application, every lender; (2) a disc passing through a plane leaving no mark — the soft credit check; (3) three stacked bars of differing length, aligned at the baseline — the real cost shown plainly; (4) two discs of equal size side by side — no charge, either kind. |
| Comparison screen fragments (×3) | Cropped hard, bleeding off one edge | Render the real interface at high fidelity in the actual palette and type, then crop: the offer list, the sort control, a single rate column. These carry more weight than any abstract render — use them wherever a section is about how the product works. |
| Use-case renders (×4) | 1:1 | Same primitive language, one idea each: converging lines for consolidation, a rising stepped form for renovation, a widening channel for business cash flow, a circle completing for life events. Resist adding literal objects. |
| Section accent renders | As needed, 1:1 | A fingerprint form for Singpass and security; three toggle forms for consent and data controls; a stepped bar comparison for rate against market. |
| Favicon and app icons | 512×512, 192×192, 32×32 | The wordmark's initial letter set in Instrument Sans on `--ink-900`. Typographic only — no bolt, no monogram device, no render. |
| OG / social card | 1200×630 | Wordmark, one line of copy, `--ink-900` ground, optionally one render at low opacity behind. |

Feed Higgsfield the hex values from the tokens section in every prompt so the generated work sits in the same palette as the build. Generate the hero loop first, lock its lighting angle, material and depth of field, then reference it in every later prompt so the whole set reads as one system rather than eleven separate images. Keep every render to two or three tones, no text, no arrows, no literal objects. If an image needs a caption to be understood, it is wrong — simplify until it does not.
